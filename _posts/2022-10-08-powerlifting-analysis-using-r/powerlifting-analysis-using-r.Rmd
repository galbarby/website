...
In this project, I'm about to analyze powerlifting competitions from the famous federation called the IPF.\
...
The meet name for example is irrelevant to the following analysis.\
...
The first analysis checks a familiar phenomenon, when males in general have stronger bodies than females.\
...
The squat is an exercise that ably demonstrates lower body strength and the bench press is an exercise that effectively demonstrates the upper body strength.\
So, in the following analysis I checked the ratio between the bench press of the males verses females and I compared it to the ratio of the males' and females' squat.\
...
Because we know that the ratio between different lifts can be influenced by gender, and females in general are lighter than males, I performed the analysis only on males.\
...
This analysis is not enough to give suggestions, because each group has individual characteristics that can affect this result.\
...
```{r message=FALSE}
repeated_dat_2 <- repeated_dat %>% filter(!is.na(WeightClassKg),!is.na(BodyweightKg))
repeated_dat_2 <- repeated_dat_2 %>% mutate(deadlift_bench_ratio=Best3BenchKg/Best3DeadliftKg)

splited_repeated <- repeated_dat_2 %>% distinct(Name,WeightClassKg,.keep_all = T) %>% 
...
For example, for an individual who has shoulder pain which is caused by stronger push muscles than pull muscles, I will suggest to consider this analysis.\
In addition, I would recommend to reduce some load on this competitor's push exercise and to lose body weight.\
...
pop <- dat %>% nrow()
pander(c(G_sq/pop,G_bp/pop,G_dl/pop))
...
squat - 67%, bench press - 45%, deadlift - 65%.\
...
One interesting thing I found that Power Lifters can use from this analysis, is the low proportion of "good lift" in the bench press.\
On average, there is more "no lift" than "good lift" in the third attempt, so power lifters should take into account this result and plan the bench press attempts accordingly.\
\
...
The first one is the oldest record in IPF (the heaviest total that no one has passed throughout the years in a specific category).\
The second is the oldest record in IPF, with more than 1,000 competitors who compete in the specific category.\
The third is the oldest competitor that ever competed on the IPF.\
You can see the impressive weight he lifted as well.\
The fourth is the heaviest total that a competitor 80 years old or more lifted in a competition.\
...
pander(records_dat %>% arrange(desc(Age)) %>% 
  select(Name,Age,BodyweightKg,WeightClassKg,Best3SquatKg,
         Best3BenchKg,Best3DeadliftKg,TotalKg) %>% slice(1))
# the heaviest total in 80 or more age group
pander(records_dat %>% filter(Age>=80) %>% slice_max(TotalKg) %>% 
  select(Name,Age,BodyweightKg,WeightClassKg,Best3SquatKg,
         Best3BenchKg,Best3DeadliftKg,TotalKg) %>% slice(1))
```
...
\
First of all, I presented the countries who have the most amount of lifters.\
...
The third analysis shows the countries with the most and the least percentages of female competitors (when taking only in consideration countries with more than 200 competitors).\

```{r}
pander(dat %>% filter(!is.na(Country)) %>% count(Country) %>% arrange(desc(n)) %>% head())
# strongest competitors
pander(dat %>% arrange(desc(Wilks)) %>% slice(1:5) %>% pull(Country) %>% unique())
#USA vs Russia
...
dat_fem <- dat_fem %>% group_by(Country) %>% summarise(femlae_precentage=mean(Sex=="F"))
pander(dat_fem %>% slice_max(femlae_precentage))
pander(dat_fem %>% slice_min(femlae_precentage))

```

As you can see, the strongest competitors in the IPF are from the USA, Russia and Poland.\
If the strength of the lifter is independent from his/her country, Poland is a surprising result because there are many countries with more competitors than Poland.\
...
The similar trends can match to the 'chisq test' in this project, which ultimately found a correlation between squat and bench press success and failure.\
